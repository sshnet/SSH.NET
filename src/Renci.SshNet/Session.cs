    ...
    public class Session : ISession
    {
        ...
        public void Connect()
        {
            ...
            try
            {
                ...
                lock (_lock)
                {
                    ...
                    if (!(serverIdentification.ProtocolVersion.Equals("2.0", StringComparison.Ordinal) || serverIdentification.ProtocolVersion.Equals("1.99", StringComparison.Ordinal)))
                    {
                        throw new SshConnectionException(string.Format(CultureInfo.CurrentCulture,
                                                                       "Server version '{0}' is not supported.",
                                                                       serverIdentification.ProtocolVersion),
        ...
        public async Task ConnectAsync(CancellationToken cancellationToken)
        {
            ...
            _socket = await _serviceFactory.CreateConnector(ConnectionInfo, _socketFactory)
                                           .ConnectAsync(ConnectionInfo, cancellationToken)
                                           .ConfigureAwait(false);

            var serverIdentification = await _serviceFactory.CreateProtocolVersionExchange()
                                                            .StartAsync(ClientVersion, _socket, cancellationToken)
                                                            .ConfigureAwait(false);
            ...
            // Start incoming request listener
            // ToDo: Make message pump async, to not consume a thread for every session
            _ = ThreadAbstraction.ExecuteThreadLongRunning(MessageListener).ConfigureAwait(false);
        ...
        private Message ReceiveMessage(Socket socket)
        {
            ...
            var serverMacLength = _serverMac is not null ? _serverMac.HashSize/8 : 0;
            ...
            if (_serverDecompression is not null)
            {
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                if (serviceAccepted is not null)
                {
                ...
                if (exceptionWaitHandle is not null)
                {
                ...
                if (keyExchangeCompletedWaitHandle is not null)
                {
                ...
                if (messageListenerCompleted is not null)
                {
