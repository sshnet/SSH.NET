...
namespace Renci.SshNet
{
    ...
    public class Session : ISession
    {
        ...
        public async Task ConnectAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                // ToDo: Migrate to SemaphoreSlim to enable async awaiting
                // ToDo: SHould we really limit this globally here, or leave it to the app?
                AuthenticationConnection.Wait();

                if (IsConnected)
                    return;

                //lock (this)   // ToDo: Why do we need double locking at all?
                {
                    // If connected don't connect again
                    if (IsConnected)
                        return;
