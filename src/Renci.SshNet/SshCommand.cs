...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
        ...
        public string Result
        {
            get
            {
                ...
                if (OutputStream is not null && OutputStream.Length > 0)
                {
                    using (var sr = new StreamReader(OutputStream, _encoding, detectEncodingFromByteOrderMarks: true, bufferSize: 1024, leaveOpen: true))
                    {
        ...
        public string Error
        {
            get
            {
                if (_hasError)
                {
                    ...
                    if (ExtendedOutputStream is not null && ExtendedOutputStream.Length > 0)
                    {
                        // do not dispose the StreamReader, as it would also dispose the stream
                        using (var sr = new StreamReader(ExtendedOutputStream, _encoding, detectEncodingFromByteOrderMarks: true, bufferSize: 1024, leaveOpen: true))
                        {
...
#pragma warning restore CA1859 // Use concrete types when possible for improved performance
        {
            ...
            if (_asyncResult is not null && !_asyncResult.EndCalled)
            {
