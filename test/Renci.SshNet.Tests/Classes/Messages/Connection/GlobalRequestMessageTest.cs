...
namespace Renci.SshNet.Tests.Classes.Messages.Connection
{
    ...
    public class GlobalRequestMessageTest : TestBase
    {
        ...
        public void Ctor_RequestNameAndWantReply()
        {
            ...
            var target = new GlobalRequestMessage(Encoding.ASCII.GetBytes(requestName), wantReply: true);
            Assert.AreEqual(requestName, target.RequestName);
            Assert.IsTrue(target.WantReply);

            target = new GlobalRequestMessage(Encoding.ASCII.GetBytes(requestName), wantReply: false);
            Assert.AreEqual(requestName, target.RequestName);
