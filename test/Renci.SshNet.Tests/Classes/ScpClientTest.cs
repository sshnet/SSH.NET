...
namespace Renci.SshNet.Tests.Classes
{
    ...
    public partial class ScpClientTest : TestBase
    {
        ...
        public void Ctor_HostAndPortAndUsernameAndPrivateKeys()
        {
            ...
            using (var client = new ScpClient(host, port, userName, privateKeys))
            {
                Assert.AreEqual(16 * 1024U, client.BufferSize);
                Assert.IsNotNull(client.ConnectionInfo);
                Assert.IsFalse(client.IsConnected);
        ...
        public void Ctor_HostAndUsernameAndPrivateKeys()
        {
            ...
            using (var client = new ScpClient(host, userName, privateKeys))
            {
                Assert.AreEqual(16 * 1024U, client.BufferSize);
                Assert.IsNotNull(client.ConnectionInfo);
                Assert.IsFalse(client.IsConnected);
        ...
        {
            using (var client = new ScpClient("HOST", 22, "USER", "PWD") { RemotePathTransformation = RemotePathTransformation.ShellQuote })
            {
                try
                {
        ...
        private static PrivateKeyFile GetRsaKey()
        {
        ...
        private static PrivateKeyFile GetDsaKey()
        {
