...
namespace Renci.SshNet.Tests.Classes.Security.Cryptography.Ciphers
{
    ...
    public class AesCipherTest : TestBase
    {
        ...
        [TestMethod]
        public void AES_CTR_OverlappingBuffers_ThrowsArgumentException()
        {
            var key = new byte[16];
            var iv = new byte[16];
            var cipher = new AesCipher(key, iv, AesCipherMode.CTR, pkcs7Padding: false);
            var buffer = new byte[64];

            Assert.ThrowsExactly<ArgumentException>(() => cipher.Encrypt(buffer, 0, 32, buffer, 16));
            Assert.ThrowsExactly<ArgumentException>(() => cipher.Decrypt(buffer, 0, 32, buffer, 16));
        }
        ...
        public void AES_ECB_128_Length16_NoPad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, iv: null, AesCipherMode.ECB, pkcs7Padding: false).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual);

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, iv: null, AesCipherMode.ECB, pkcs7Padding: false).Encrypt(actual, 0, input.Length, actual, 0);

            Assert.AreEqual(expected.Length, encryptCount);
            CollectionAssert.AreEqual(expected, actual);
        }
        ...
        public void AES_ECB_128_Length16_Pad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, iv: null, AesCipherMode.ECB, pkcs7Padding: true).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual.Take(decryptCount));

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, iv: null, AesCipherMode.ECB, pkcs7Padding: true).Encrypt(actual, 0, input.Length, actual, 0);
        ...
        public void AES_CBC_128_Length16_NoPad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CBC, pkcs7Padding: false).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual);

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CBC, pkcs7Padding: false).Encrypt(actual, 0, input.Length, actual, 0);
        ...
        public void AES_CBC_128_Length16_Pad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CBC, pkcs7Padding: true).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual.Take(decryptCount));

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CBC, pkcs7Padding: true).Encrypt(actual, 0, input.Length, actual, 0);
        ...
        public void AES_CFB_128_Length16_NoPad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CFB, pkcs7Padding: false).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual);

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CFB, pkcs7Padding: false).Encrypt(actual, 0, input.Length, actual, 0);
        ...
        public void AES_CTR_128_Length16_NoPad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CTR, pkcs7Padding: false).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual);

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.CTR, pkcs7Padding: false).Encrypt(actual, 0, input.Length, actual, 0);
        ...
        public void AES_OFB_128_Length16_NoPad()
        {
            ...
            // Decrypt in-place
            var decryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.OFB, pkcs7Padding: false).Decrypt(actual, 0, actual.Length, actual, 0);

            Assert.AreEqual(input.Length, decryptCount);
            CollectionAssert.AreEqual(input, actual);

            // Re-encrypt in-place
            var encryptCount = new AesCipher(key, (byte[])iv.Clone(), AesCipherMode.OFB, pkcs7Padding: false).Encrypt(actual, 0, input.Length, actual, 0);
