...
namespace Renci.SshNet.Tests.Classes.Security.Cryptography
{
    ...
    public class RsaDigitalSignatureTest : TestBase
    {
        ...
        public void Sha1_SignAndVerify()
        {
            ...
            var rsaPublicKey = new RsaKey { Public = rsaKey.Public };
            var digitalSignaturePublic = new RsaDigitalSignature(rsaPublicKey);
            Assert.IsTrue(digitalSignaturePublic.Verify(data, signedBytes));

            rsaPublicKey.Dispose();
            digitalSignature.Dispose();
            digitalSignaturePublic.Dispose();
        }
        ...
        public void Sha256_SignAndVerify()
        {
            ...
            // 'Workaround': use a key with no private key information
            var rsaPublicKey = new RsaKey { Public = rsaKey.Public };
            var digitalSignaturePublic = new RsaDigitalSignature(rsaPublicKey, HashAlgorithmName.SHA256);
            Assert.IsTrue(digitalSignaturePublic.Verify(data, signedBytes));
        ...
        public void Sha512_SignAndVerify()
        {
            ...
            // 'Workaround': use a key with no private key information
            var rsaPublicKey = new RsaKey { Public = rsaKey.Public };
            var digitalSignaturePublic = new RsaDigitalSignature(rsaPublicKey, HashAlgorithmName.SHA512);
            Assert.IsTrue(digitalSignaturePublic.Verify(data, signedBytes));
