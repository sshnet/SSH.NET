...
namespace Renci.SshNet.Tests.Classes.Security
{
    ...
    public class KeyHostAlgorithmTest : TestBase
    {
        ...
        {
            using (var rsaKey = GetRsaKey())
            {
                var keyHostAlgorithm = new KeyHostAlgorithm("ssh-rsa", rsaKey);
        ...
        {
            using (var rsaKey = GetRsaKey())
            using (var rsaDigitalSignature = new RsaDigitalSignature(rsaKey, HashAlgorithmName.SHA256))
            {
                var keyHostAlgorithm = new KeyHostAlgorithm("rsa-sha2-256", rsaKey, rsaDigitalSignature);
        ...
        public void SshRsa_SignAndVerify()
        {
            ...
            using (var key = GetRsaKey())
            {
                var keyAlgorithm = new KeyHostAlgorithm("ssh-rsa", key);
            ...
            {
                var keyAlgorithm = new KeyHostAlgorithm("ssh-rsa", key, GetRsaPublicKeyBytes());
        ...
        public void RsaSha256_SignAndVerify()
        {
            ...
            using (var key = GetRsaKey())
            using (var digitalSignature = new RsaDigitalSignature(key, HashAlgorithmName.SHA256))
            {
                var keyAlgorithm = new KeyHostAlgorithm("rsa-sha2-256", key, digitalSignature);
            ...
            using (var digitalSignature = new RsaDigitalSignature(key, HashAlgorithmName.SHA256))
            {
                var keyAlgorithm = new KeyHostAlgorithm("rsa-sha2-256", key, GetRsaPublicKeyBytes(), digitalSignature);
        ...
        public void RsaSha512_SignAndVerify()
        {
            ...
            using (var key = GetRsaKey())
            using (var digitalSignature = new RsaDigitalSignature(key, HashAlgorithmName.SHA512))
            {
                var keyAlgorithm = new KeyHostAlgorithm("rsa-sha2-512", key, digitalSignature);
            ...
            using (var digitalSignature = new RsaDigitalSignature(key, HashAlgorithmName.SHA512))
            {
                var keyAlgorithm = new KeyHostAlgorithm("rsa-sha2-512", key, GetRsaPublicKeyBytes(), digitalSignature);
                Assert.IsTrue(keyAlgorithm.VerifySignature(data, expectedEncodedSignatureBytes));
            }
        }
        ...
        private static RsaKey GetRsaKey()
        {
            ...
            using (var privateKey = new PrivateKeyFile(stream))
            {
                return (RsaKey) privateKey.Key;
            }
        }

        private static void TestRsaPublicKeyData(string signatureIdentifier, HashAlgorithmName hashAlgorithmName)
        {
            using (var key = GetRsaKey())
            using (var digitalSignature = new RsaDigitalSignature(key, hashAlgorithmName))
            {
                var keyAlgorithm = new KeyHostAlgorithm(signatureIdentifier, key, digitalSignature);
