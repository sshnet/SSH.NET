...
namespace Renci.SshNet.Tests.Classes
{
    ...
    public class ShellStreamTest : TestBase
    {
        ...
        public void ReadLine_MultiByteCharacters()
        {
            ...
            using (var shellStream = CreateShellStream())
            {
        ...
        private ShellStream CreateShellStream()
        {
            ...
            _connectionInfoMock.Setup(p => p.Encoding)
                               .Returns(_encoding);
            _sessionMock.Setup(p => p.CreateChannelSession())
                        .Returns(_channelSessionMock.Object);
            _channelSessionMock.Setup(p => p.Open());
            _channelSessionMock.Setup(p => p.SendPseudoTerminalRequest(_terminalName,
                                                                       _widthColumns,
                                                                       _heightRows,
                                                                       _widthPixels,
                                                                       _heightPixels,
